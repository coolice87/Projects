 ...
 *
 * \retval TRUE  ：检测到上升沿
 * \retval FALSE ：没有检测到上升沿或者上升沿被清除
 */
am_static_inline 
bool_t amhw_lpc82x_pint_rise_dec_get (amhw_lpc82x_pint_t *p_hw_pint,
                                      uint8_t             chan)
{
    return (p_hw_pint->rise & AM_BIT(chan)) ? TRUE : FALSE;
}
 ...
 *
 * \retval TRUE  ：检测到下降沿
 * \retval FALSE ：没有检测到下降沿或者下降沿被清除
 */
am_static_inline 
bool_t amhw_lpc82x_pint_fall_dec_get (amhw_lpc82x_pint_t *p_hw_pint,
                                      uint8_t             chan)
{
    return ((p_hw_pint->fall & AM_BIT(chan)) ? TRUE : FALSE);
}
 ...
 * \param[in] chan      : 中断通道编号
 *
 * \retval TRUE  ：通道中断中断正在请求
 * \retval FALSE ：没有中断请求
 */
am_static_inline 
bool_t amhw_lpc82x_pint_int_stat_get (amhw_lpc82x_pint_t *p_hw_pint, 
                                      uint8_t             chan)
{
    return (p_hw_pint->ist & AM_BIT(chan)) ? TRUE : FALSE;
}
 ...
 */
am_static_inline 
bool_t amhw_lpc82x_pint_pmatch_get (amhw_lpc82x_pint_t          *p_hw_pint, 
                                    amhw_lpc82x_pint_bitslice_t  slice)
{
    return (p_hw_pint->pmctrl & AM_BIT(slice + 24)) ? TRUE : FALSE;
}
 ...
 * \param[in] chan      : 中断通道
 * \param[in] arg       : 位片参数
 * \param[in] is_end    : 如果该值为TURE，当前位片即为最终构成
 *
 * \return 无
 */
