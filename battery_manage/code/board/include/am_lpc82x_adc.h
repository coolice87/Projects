...
typedef struct am_lpc82x_adc_devinfo {
     ...
     * \brief ADC工作模式
     *
     * 参数用法，\ref grp_am_lpc82x_adc_work_mode
     *         使用#AM_LPC82X_ADC_WORK_MODE_DMA，
     *             调用am_adc_start()接口时，缓冲区描述个数（desc_num）需小于等于2；
     *         使用#AM_LPC82X_ADC_WORK_MODE_INT，
     *             调用am_adc_start()接口时，缓冲区描述个数无限制，且dma_chan被忽略。
     */
    int16_t    work_mode;
 ...
 * 默认去初始化序列A 
 *
 * \param[in] handle : am_lpc82x_adc_init() 初始化函数获得的ADC服务句柄
 *
 * \return 无 
 */
