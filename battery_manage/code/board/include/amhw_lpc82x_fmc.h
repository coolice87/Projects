...
{
    AMHW_LPC82X_FMC->flashcfg = time & 0x3;
}
...
{
    return (amhw_lpc82x_fmc_time_t)(AMHW_LPC82X_FMC->flashcfg & 0x3);
}
...
{
    AMHW_LPC82X_FMC->fmsstart = (addr >> 4) & 0x1ffff;
}
...
{
    return ((AMHW_LPC82X_FMC->fmsstart) & 0x1ffff) << 4;
}
...
{
    AMHW_LPC82X_FMC->fmsstop = (addr >> 4) & 0x1ffff;
}
...
{
    return ((AMHW_LPC82X_FMC->fmsstop) & 0x1ffff) << 4;
}
...
{
    AMHW_LPC82X_FMC->fmsstop |= (1UL << 31);
}
...
{
    AMHW_LPC82X_FMC->fmsstart = (start_addr >> 4) & 0x1ffff;
    AMHW_LPC82X_FMC->fmsstop  = ((end_addr >> 4) & 0x1ffff) | 0x80000000;
    return AMHW_LPC82X_FMC->fmsw0;
}
