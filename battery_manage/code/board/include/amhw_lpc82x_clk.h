...
{
    AMHW_LPC82X_SYSCON->syspllctrl = AM_SBF(psel, 5) | msel;
}
...
{
    AMHW_LPC82X_SYSCON->syspllclksel = (uint32_t)src;
    
    /* 更新PLL时钟 */
    AMHW_LPC82X_SYSCON->syspllclkuen = 0x00;
    AMHW_LPC82X_SYSCON->syspllclkuen = 0x01;
}    
...
{
    AM_BIT_SET(AMHW_LPC82X_SYSCON->sysahbclkctrl, clk);
}
...
{
    AM_BIT_CLR(AMHW_LPC82X_SYSCON->sysahbclkctrl, clk);
}
 ...
 */
am_static_inline
bool_t amhw_lpc82x_clk_pll_locked_chk (void)
{
    return (bool_t)(AMHW_LPC82X_SYSCON->syspllstat & 0x01);
}

/**
 * \brief 系统振荡器旁路控制使能/禁能
 *
 * \param[in] bypass : 旁路系统振荡器 FALSE-禁能,TURE-使能
 * \param[in] highfr : 振荡器频率范围 FALSE: 1-20MHz TRUE:15-25MHz
 *
 * \return 无
 */
am_static_inline
void amhw_lpc82x_clk_pll_bypass_set (bool_t bypass, bool_t highfr)
{
    if (bypass) {
        AM_BIT_SET(AMHW_LPC82X_SYSCON->sysoscctrl, 0);
    } else {
        AM_BIT_CLR(AMHW_LPC82X_SYSCON->sysoscctrl, 0);
    }
    
    if (highfr) {
        AM_BIT_SET(AMHW_LPC82X_SYSCON->sysoscctrl, 1);
    } else {
        AM_BIT_CLR(AMHW_LPC82X_SYSCON->sysoscctrl, 1);
    }
}
...
{
    AMHW_LPC82X_SYSCON->mainclksel = (uint32_t)src;
    
    /* 更新main时钟 */
    AMHW_LPC82X_SYSCON->mainclkuen = 0x00;
    AMHW_LPC82X_SYSCON->mainclkuen = 0x01;
}
...
{
    return (amhw_lpc82x_clk_main_src_t)(AMHW_LPC82X_SYSCON->mainclksel);
}
...
void amhw_lpc82x_clk_wdtoscc_cfg (uint8_t rate, 
                                  uint8_t div)
{                                          
    AMHW_LPC82X_SYSCON->wdtoscctrl = AM_SBF((uint8_t) (rate), 5) |
                                     ((div >> 1) - 1);
}
...
{
    AMHW_LPC82X_SYSCON->sysahbclkdiv = div;
}
...
{
    return AMHW_LPC82X_SYSCON->sysahbclkdiv & 0xFF;
}
...
{
    AMHW_LPC82X_SYSCON->uartclkdiv = div;
}
...
{
    AMHW_LPC82X_SYSCON->uartfrgdiv  = fdiv;
    AMHW_LPC82X_SYSCON->uartfrgmult = fmul;
}
                                    ...
                                    uint8_t                     div)
{
    ...
    AMHW_LPC82X_SYSCON->ioconclkdiv[t_divider] = div;
}
...
{
    int32_t div = AMHW_LPC82X_SYSCON->ioconclkdiv[divider] & 0xFF;
