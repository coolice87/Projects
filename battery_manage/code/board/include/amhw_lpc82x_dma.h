 ...
 * \param[in] chan     : DMA通道编号，DMA_CHAN_*宏值 (#DMA_CHAN_0)
 * 
 * \retval TRUE  : 通道处于活动状态
 * \retval FALSE : 通道处于非活动状态
 *
 * \note 启动DMA后，通道处于活跃状态；DMA全部传输完成，通道处于非活跃状态
 */
am_static_inline
bool_t amhw_lpc82x_dma_chan_isactive (amhw_lpc82x_dma_t *p_hw_dma, uint8_t chan)
{
    return ((p_hw_dma->active0 & AM_BIT(chan)) ? TRUE : FALSE);
}
 ...
 * 
 * \retval TRUE  : 通道 busy
 * \retval FALSE : 通道 unbusy
 *
 * \note DMA通道在传输过程中被软件禁能，调用此函数判断是否可以abort终止
 */
am_static_inline
bool_t amhw_lpc82x_dma_chan_isbusy (amhw_lpc82x_dma_t *p_hw_dma, uint8_t chan)
{
    return ((p_hw_dma->busy0 & AM_BIT(chan)) ? TRUE : FALSE);
}
 ...
 *
 * \retval TRUE  : 通道发生错误
 * \retval FALSE : 通道没有发生错误
 */
am_static_inline
bool_t amhw_lpc82x_dma_chan_iserror (amhw_lpc82x_dma_t *p_hw_dma,
                                     uint8_t            chan)
{
    return ((p_hw_dma->errint0 & AM_BIT(chan)) ? TRUE : FALSE);
}
 ...
 * \param[in] chan     : DMA通道编号，DMA_CHAN_*宏值 (#DMA_CHAN_0)
 * 
 * \retval TRUE  : 通道的A中断标志置位
 * \retval FALSE : 通道的A中断标志没有被置位
 */
am_static_inline
bool_t amhw_lpc82x_dma_chan_inta_isactive (amhw_lpc82x_dma_t *p_hw_dma,
                                           uint8_t            chan)
{
    return ((p_hw_dma->inta0 & AM_BIT(chan)) ? TRUE : FALSE);
}
 ...
 * \param[in] chan     : DMA通道编号，DMA_CHAN_*宏值 (#DMA_CHAN_0)
 *
 * \retval TRUE  : 通道的B中断标志置位
 * \retval FALSE : 通道的B中断标志没有被置位
 */
am_static_inline
bool_t amhw_lpc82x_dma_chan_intb_isactive (amhw_lpc82x_dma_t *p_hw_dma,
                                           uint8_t            chan)
{
    return ((p_hw_dma->intb0 & AM_BIT(chan)) ? TRUE : FALSE);
}
 ...
 * \param[in] chan     : DMA通道编号，DMA_CHAN_*宏值 (#DMA_CHAN_0)
 *
 * \retval TRUE  : 传输描述符有效标志置位
 * \retval FALSE : 传输描述符有效标志没有置位
 */
am_static_inline
bool_t amhw_lpc82x_dma_chan_xferdesc_isvalid (amhw_lpc82x_dma_t *p_hw_dma,
                                              uint8_t            chan)
{
    return ((p_hw_dma->chan[chan].ctlstat & AM_BIT(0)) ? TRUE : FALSE);
}
 ...
 * \param[in] chan     : DMA通道编号，DMA_CHAN_*宏值 (#DMA_CHAN_0)
 *
 * \retval TRUE  : 当前通道触发被置位
 * \retval FALSE : 当前通道触发未被置位
 */
am_static_inline
bool_t amhw_lpc82x_dma_chan_xferdesc_istriggered (amhw_lpc82x_dma_t *p_hw_dma,
                                                  uint8_t            chan)
{
    return ((p_hw_dma->chan[chan].ctlstat & AM_BIT(2)) ? TRUE : FALSE);
}
