...
{
    AMHW_LPC82X_SYSCON->sysmemremap = (uint32_t)remap;
}
...
{
    return (amhw_lpc82x_syscon_bootmode_remap_t)AMHW_LPC82X_SYSCON->sysmemremap;
}
...
{
    AMHW_LPC82X_SYSCON->systckcal = systickcal_val;
}
...
{
    return AMHW_LPC82X_SYSCON->systckcal;
}
...
{
    return AMHW_LPC82X_SYSCON->sysrststat;
}
...
{
    AMHW_LPC82X_SYSCON->sysrststat = reset;
}
...
{
    AM_BIT_CLR(AMHW_LPC82X_SYSCON->presetctrl, periph);
    AM_BIT_SET(AMHW_LPC82X_SYSCON->presetctrl, periph);
}
...
{
    AMHW_LPC82X_SYSCON->irqlatency = latency;
}
...
{
    return (AMHW_LPC82X_SYSCON->irqlatency & 0xFF);
}
...
{
    /* 先清除NMI使能位，后写入时钟源 */
    AM_BIT_CLR(AMHW_LPC82X_SYSCON->nmisrc, 31);
    AMHW_LPC82X_SYSCON->nmisrc  = intsrc;
}
...
{
    AM_BIT_SET(AMHW_LPC82X_SYSCON->nmisrc, 31);
}
...
{
    if (pid < 32) {
        AM_BIT_SET(AMHW_LPC82X_SYSCON->starterp0, pid);
    } else {
        AM_BIT_SET(AMHW_LPC82X_SYSCON->starterp1, pid - 32);
    }
}
...
{
    if (pid < 32) {
        AM_BIT_CLR(AMHW_LPC82X_SYSCON->starterp0, pid);
    } else {
        AM_BIT_CLR(AMHW_LPC82X_SYSCON->starterp1, pid - 32);
    }
}
...
{
    AM_BIT_CLR_MASK(AMHW_LPC82X_SYSCON->pdsleepcfg, flags);
}
...
{
    AM_BIT_SET_MASK(AMHW_LPC82X_SYSCON->pdsleepcfg, flags);
}
...
{
    AMHW_LPC82X_SYSCON->pdawakecfg = flags;
}
...
{
    AM_BIT_CLR_MASK(AMHW_LPC82X_SYSCON->pdawakecfg, wakeupflags);
}
...
{
    AM_BIT_SET_MASK(AMHW_LPC82X_SYSCON->pdawakecfg, wakeupflags);
}
...
{
    return AMHW_LPC82X_SYSCON->pdruncfg;
}
...
{
    AM_BIT_SET_MASK(AMHW_LPC82X_SYSCON->pdruncfg, powerdownmask);
}
...
{
    AM_BIT_CLR_MASK(AMHW_LPC82X_SYSCON->pdruncfg, powerupmask);
}
 ...
 */
am_static_inline
bool_t amhw_lpc82x_syscon_power_check (uint32_t powermask)
{
    return (bool_t)((AMHW_LPC82X_SYSCON->pdruncfg & powermask) == 0);
}
...
{
    return AMHW_LPC82X_SYSCON->deviceid;
}
...
{                         
    AMHW_LPC82X_SYSCON->pintsel[pint_sel] = pin;
}
...
{
    AMHW_LPC82X_SYSCON->bodctrl = ((uint32_t)rstlvl) | AM_SBF(intlvl, 2);
}
...
{
    AM_BIT_SET(AMHW_LPC82X_SYSCON->bodctrl, 4);
}
...
{
    AM_BIT_CLR(AMHW_LPC82X_SYSCON->bodctrl, 4);
}
