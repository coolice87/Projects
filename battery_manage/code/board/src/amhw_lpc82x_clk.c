...
{     
    ...
    switch ((amhw_lpc82x_pllinclk_src_t) (AMHW_LPC82X_SYSCON->syspllclksel &
             0x03)) {
...
uint32_t amhw_lpc82x_clk_pllout_rate_get (void)
{
    ...
    syspllout_rate = amhw_lpc82x_clk_pllin_rate_get() *
                     ((AMHW_LPC82X_SYSCON->syspllctrl & 0x01F) + 1);
...
uint32_t amhw_lpc82x_clk_wdt_rate_get (void)
{
    ...
    /* 得到时钟频率*/
    switch ((AMHW_LPC82X_SYSCON->wdtoscctrl >> 5) & 0xF) {
    ...
    g_wdtosc_clkfreq /= (((AMHW_LPC82X_SYSCON->wdtoscctrl & 0x1F) + 1) << 1);
...
{
    /* CLKOUT 时钟源选择 */
    AMHW_LPC82X_SYSCON->clkoutsel = src;
    
    /* 更新CLKOUT时钟 */
    AMHW_LPC82X_SYSCON->clkoutuen = 0x00;
    AMHW_LPC82X_SYSCON->clkoutuen = 0x01;
    
    /* CLKOUT 分频系数*/
    AMHW_LPC82X_SYSCON->clkoutdiv = div;         
}
...
uint32_t amhw_lpc82x_clk_usart_baseclkrate_get (void)
{
    ...
    /* 得到USART小数分频系数*/
    div = AMHW_LPC82X_SYSCON->uartclkdiv;
    if (div == 0) {    
       ...
       inclk = 0; 
        ...
        if ((AMHW_LPC82X_SYSCON->uartfrgdiv & 0xFF) == 0xFF) {
            
            /* 小数部分使能，得到倍频系数*/
            mult = AMHW_LPC82X_SYSCON->uartfrgmult & 0xFF;
...
int amhw_lpc82x_clk_usart_baseclkrate_set (uint32_t rate)
{
    ...
    /* 保留小数部分 */
    AMHW_LPC82X_SYSCON->uartfrgmult = (uint8_t) mult;
    AMHW_LPC82X_SYSCON->uartfrgdiv  = 0xFF;
