...
#define __DMA_CHAN_IS_ACTIVE(p_hw_dma,chan) \
           (amhw_lpc82x_dma_chan_isactive((p_hw_dma), (chan))      == TRUE || \
            amhw_lpc82x_dma_chan_iserror((p_hw_dma), (chan))       == TRUE || \
            amhw_lpc82x_dma_chan_inta_isactive((p_hw_dma), (chan)) == TRUE || \
            amhw_lpc82x_dma_chan_intb_isactive((p_hw_dma), (chan)) == TRUE)
                            ...
                            void                       *p_arg)
{
    ...
    if (AM_BIT_GET(p_ctr->ext_info,
                   __DMA_EXT_INFO_HWTRIG_POS) == FALSE) {
                                     ...
                                     void                        *p_arg)
{
    ...
    if(AM_BIT_GET(p_ctr->ext_info, __DMA_EXT_INFO_REPEAT_POS) == TRUE) {
