 ...
 * \brief 转换结果数据左对齐(默认)
 *
 * 此时，数据缓冲区类型必须为：am_adc_val_t，数据按照左对齐进行存放。
 * am_adc_val_t类型的数据位数超过ADC转换位数时，数据低位补0。
 */
#define AM_ADC_DATA_ALIGN_LEFT    0x00

/** 
 * \brief 转换结果数据右对齐，这种方式往往能够取得更快的速度
 *
 * 此时，数据缓冲区类型需根据实际的ADC转换位数(可由 am_adc_bits_get()获取)设置:
 *
 ...
 * \param[in] p_buf        : 数据缓冲区，具体类型的设置可参见本函数的说明
 * \param[in] length       : 数据缓冲区的长度，决定了可以存放多少次转换结果
 * \param[in] pfn_complete : 本次数据缓冲区填充满后的回调函数
 * \param[in] p_arg        : 回调函数用户自定义参数
 *
 * \note 关于p_buf数据缓冲区的类型，具体类型应根据需要设置：
 * - 数据默认按照左对齐，类型应为 am_adc_val_t，数据位数不足时低位补0。
 * - 如需要数据右对齐，则可以在启动ADC(am_adc_start())时设定flags标志: 
 *   #AM_ADC_DATA_ALIGN_RIGHT
 *   此时：类型需根据实际的ADC转换位数(可由 am_adc_bits_get()获取)设置:
 *
 *      ADC bits    |    buffer type
 *    ------------- | -------------------
 *       1  ~  8    |  uint8_t/int8_t
 ...
 *      17  ~  31   |  uint32_t/int32_t
 *
 * 数据缓冲区类型的数据位数超过ADC转换位数时，数据高位补0。如ADC位数为10位，则
 * 应该使用 uint16_t 类型的数据缓冲区，转换结束后，数据低10位为有效数据，高6位
 * 全部为0。
 */
am_static_inline
 ...
 *  am_adc_val_t        adc_val0[80];           // 数据缓冲区1
 *  am_adc_val_t        adc_val1[80];           // 数据缓冲区2
 *  
 ...
 * \param[in] p_val    : 转换结果存放的缓冲区，数据左对齐
...
/**
 * \brief 读取指定通道的电压值（单位：mV）
 *
 * \param[in] handle : ADC标准服务操作句柄
 * \param[in] chan   : ADC通道号
 * \param[in] p_mv   : 转换结果(电压值)存放的缓冲区
 * \param[in] length : 缓冲区的长度
 *
 * \retval   AM_OK     : 操作成功
 * \retval  -AM_ENXIO  : ADC通道号不存在
 ...
 * \param[in] handle : ADC标准服务操作句柄
 * \param[in] chan   : ADC通道号
 * \param[in] p_val  : 转换结果采样值存放的缓冲区
 * \param[in] length : 缓冲区的长度
 * \param[in] p_mv   ：电压值存放缓冲区，有时候为节省内存，如果不需要保存采样值，
 *                     则可以将该值设置为与p_val的值相等。
 *
 * \retval   AM_OK     : 操作成功
 * \retval  -AM_ENXIO  : ADC通道号不存在
 * \retval  -AM_EINVAL : 无效参数
 *     
 * \note 仅支持默认情况下，进行右对齐转换得到的数据，即转换结果的数据缓冲区类型
 * 为am_adc_val_t。当使用右对齐方式得到的数据转换结果时，可以使用如下方式得到
 * 采样值对应的电压值：
